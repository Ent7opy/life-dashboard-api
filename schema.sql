...
-- Run this migration after connecting to database
...
CREATE TABLE users (
  ...
  email TEXT UNIQUE,
  api_key_hash TEXT, -- bcrypt hash of API key (if using API key auth)
...
CREATE TABLE university_path (
  ...
  UNIQUE(user_id) -- one active path per user
);
...
CREATE INDEX idx_weekly_review_entry_date ON weekly_review(entry_date);
...
-- Insert a default user (API key will be generated by app)
-- Password/API key will be set via environment variable
